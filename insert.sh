...
if ! [[ -f "db/$db_name/.meta_$checktablename" ]]; then
	whiptail --title "Error Message" --msgbox "Table $checktablename you entered doesn't exist, choose another one " 8 45
       checktablename=$(whiptail --title "Choose Table" --inputbox "Enter Table Name" 8 45 3>&1 1>&2 2>&3)
fi


checkcolsno=`awk 'END {print NR}' "db/$db_name/.meta_$checktablename"` # check in metadata file
sep=":"
echo $checkcolsno
...
for (( i=1 ; i <= $checkcolsno ; i++ )); do
	...
	checkcolname=`awk 'BEGIN {FS=":"}{if ( NR=='$i' ) print $1 }' "db/$db_name/.meta_$checktablename"` # id , name, ... => col Name
	checkdatatype=`awk 'BEGIN {FS=":"}{if ( NR=='$i' ) print $2 }' "db/$db_name/.meta_$checktablename"`  # int or str or boolen
	checkisprimary=`awk 'BEGIN {FS=":"}{if ( NR=='$i' ) print $3 }' "db/$db_name/.meta_$checktablename"` #PK OR ""
