...
do
	...
	#read db_name 
	db_name=$(whiptail --title "Create Databse Message" --inputbox "Enter DB name  [0 -> exit]: "  8 45 3>&1 1>&2 2>&3)
	if [[ $db_name == "0" ]]
	then
		break
	fi
	...
	then
		...
		then
			...
			whiptail --title "Create Databse Message" --msgbox "DataBase [$db_name] is already exists." 8 45
		else
			green "$db_name is valid"
			mkdir -p ./db/$db_name
			whiptail --title "Create Databse Message" --msgbox "DataBase [$db_name] successfully created." 8 45
			break
		fi
	else
		clear
